...
import WxCheckbox from 'element-ui/packages/checkbox';
import WxTooltip from 'element-ui/packages/tooltip';
...
export default {
  ...
  watch: {
    ...
    // update DOM manually. see https://github.com/WxemeFE/element/pull/13954/files#diff-9b450c00d0a9dec0ffad5a3176972e40
  ...
  methods: {
    ...
    rowRender(row, $index, treeRowData) {
      ...
      // 使用 :style 代替 v-show https://github.com/WxemeFE/element/issues/16995
    ...
    wrappedRowRender(row, $index) {
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        if (!renderExpanded) {
          console.error('[Wxement Error]renderExpanded is required.');
          return tr;
        }
        // 使用二维数组，避免修改 $index
        return [[
          ...
          <tr key={'expanded-row__' + tr.key}>
            <td colspan={ this.columnsCount } class="el-table__cell el-table__expanded-cell">
              { renderExpanded(this.$createWxement, { row, $index, store: this.store }) }
            </td>
          </tr>]];
