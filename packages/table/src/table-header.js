...
export default {
  ...
  mounted() {
    // nextTick 是有必要的 https://github.com/WxemeFE/element/pull/11311
    this.$nextTick(() => {
  ...
  methods: {
    ...
    handleFilterClick(event, column) {
      ...
      if (!filterPanel) {
        ...
        !this.$isServer && filterPanel.$mount(document.createWxement('div'));
      }
    ...
    handleMouseDown(event, column) {
      ...
      if (this.draggingColumn && this.border) {
        ...
        const table = this.$parent;
        const tableWx = table.$el;
        const tableLeft = tableWx.getBoundingClientRect().left;
        const columnWx = this.$el.querySelector(`th.${column.id}`);
        const columnRect = columnWx.getBoundingClientRect();
        const minLeft = columnRect.left - tableLeft + 30;

        addClass(columnWx, 'noclick');
