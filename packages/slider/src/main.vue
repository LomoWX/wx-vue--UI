...
<script type="text/babel">
  import WxInputNumber from 'element-ui/packages/input-number';
    ...
    methods: {
      ...
      setValues() {
        if (this.min > this.max) {
          console.error('[Wxement Error][Slider]min should not be greater than max.');
          return;
        }
        ...
        if (this.range && Array.isArray(val)) {
          if (val[1] < this.min) {
            ...
            if (this.valueChanged()) {
              this.dispatch('WxFormItem', 'el.form.change', [this.minValue, this.maxValue]);
    ...
    computed: {
      stops() {
        ...
        if (this.step === 0) {
          ...
          console.warn('[Wxement Warn][Slider]step should not be 0.');
          return [];
        }
