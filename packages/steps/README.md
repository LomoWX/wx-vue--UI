...
```javascript
import Vue from 'vue'
import WxStep from 'element-steps'
...
```javascript
import Vue from 'vue'
import { WxSteps, WxStep } from 'element-steps'

Vue.component('el-steps', WxSteps)
Vue.component('el-step', WxStep)
```
...
| icon | 图标 | Wxement Icon 提供的图标，如果要使用自定义图标可以通过 slot 方式写入 | string | — |
